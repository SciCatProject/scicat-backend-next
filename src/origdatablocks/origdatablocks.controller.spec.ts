...
describe("OrigDatablocksController", () => {
  ...
  beforeEach(async () => {
    ...
    origDatablocksService = module.get<OrigDatablocksService>(OrigDatablocksService);
  ...
  describe("update", () => {
    ...
    it("should return updated datablock on success", async () => {
      ...
      origDatablocksService.findOne.mockResolvedValue(mockDatablock);
      origDatablocksService.findByIdAndUpdate.mockResolvedValue(updatedDatablock);

      const result = await controller.update(mockRequest, "123", mockHeaders, mockDto);

      expect(result).toEqual(updatedDatablock);
      expect(controller["updateDatasetSizeAndFiles"]).toHaveBeenCalledWith("ds1");
    });
    ...
    describe("update", () => {
      ...
      it("should proceed with update if 'if-unmodified-since' header is missing", async () => {
        ...
        const result = await controller.update(mockRequest, "123", headers, mockDto);
