...
describe("SamplesController", () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(controller as any, "checkPermissionsForSample").mockResolvedValue(true);
  });
  ...
  describe("update", () => {
    ...
    it("should update sample if header date is invalid", async () => {
      ...
      const result = await controller.update(mockRequest, sampleId, updateDto, headers);
      expect(result).toEqual({...sample, ...updateDto});
    });
