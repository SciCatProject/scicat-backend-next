...
describe('AttachmentsController - findOneAndUpdate', () => {
  ...
  beforeEach(async () => {
      ...
      providers: [
        {
          ...
          useValue: {
            findOneAndUpdate: jest.fn().mockResolvedValue(mockUpdatedAttachment),
          },
        },
        {
    ...
    jest.spyOn(controller, 'checkPermissionsForAttachment').mockResolvedValue(mockAttachment);
  });
  ...
  it('should update attachment with application/merge-patch+json', async () => {
    ...
    expect(service.findOneAndUpdate).toHaveBeenCalledWith({_id: '123'}, expectedPatched);
  });
  ...
  it('should throw PRECONDITION_FAILED if If-Unmodified-Since is older than updatedAt', async () => {
    ...
    await expect(
      controller.findOneAndUpdate({headers} as any, '123', headers, dto),
    ).rejects.toThrow(
      new HttpException(
