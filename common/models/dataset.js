...
module.exports = function(Dataset) {
    ...
    // Dataset.beforeRemote("findOne", function(ctx, instance, next) {
    //     console.log(">>> Received Filter: ", ctx.args.filter);
    //     if (!ctx.args.filter.where.isPublished) {
    //         let modifiedWhere = {};
    //         Object.keys(ctx.args.filter.where).forEach(key => {
    //             if (key !== "isPublished") {
    //                 modifiedWhere[key] = ctx.args.filter.where[key];
    //             }
    //         });
    //         ctx.args.filter.where = modifiedWhere;
    //     }
    //     next();
    // });
