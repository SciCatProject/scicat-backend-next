...
class OrigDatablock(object):
    ...
    def dataFileList(self):
        ...
        List of files contained in the linked dataset. Files can be regular files, folders and softlinks. In case the linked dataset has a source *folder* all file paths are relative file paths. In the single file dataset case the path is the fullpath to the dataset file on the source system.
