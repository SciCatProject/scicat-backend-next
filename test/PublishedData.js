...
describe.only("1600: PublishedData: Test of access to published data", () => {
  ...
  it("0020: should not be able to fetch this new published data in private state", async () => {
    return request(appUrl)
  ...
  it("0040: should be able to make published data public", async () => {
    return request(appUrl)
      .post("/api/v3/PublishedData/" + doi + "/publish")
      .set("Accept", "application/json")
      .set({ Authorization: `Bearer ${accessTokenAdminIngestor}` })
      .expect(TestData.EntryCreatedStatusCode)
      .expect("Content-Type", /json/)
  ...
  it("0060: should amend this new registered published data", async () => {
    return request(appUrl)
      .post("/api/v3/PublishedData/" + doi + "/amend")
      .set("Accept", "application/json")
      .set({ Authorization: `Bearer ${accessTokenAdminIngestor}` })
      .expect(TestData.EntryCreatedStatusCode)
      .expect("Content-Type", /json/);
