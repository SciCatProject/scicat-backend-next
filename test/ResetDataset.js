...
var testDataBlock = {
    archiveId:
        "oneCopyBig/p10029/raw/2018/01/23/20.500.11935/07e8a14c-f496-42fe-b4b4-9ff41061695e_1_2018-01-23-03-11-34.tar",
    size: 41780190,
    packedSize: 41780190,
    chkAlg: "sha1",
    version: "0.6.0",
...
describe("Create Dataset and its Datablocks DatasetLifecycle, then reset Datablocks and Datasetlifecycle status", () => {
    ...
    it("should retrieve existing Datablocks with specific archiveId, if any", function(done) {
        request(app)
            .get(
                "/api/v2/Datablocks?filter=%7B%22where%22%3A%7B%22archiveId%22%3A%22" +
                    encodeURIComponent(testDataBlock.archiveId) +
                    "%22%7D%7D&access_token=" +
                    accessTokenIngestor
            )
    ...
    it("adds a new datablock", function(done) {
        request(app)
            .post(
                "/api/v2/Datablocks?access_token=" + accessTokenArchiveManager
            )
            .send(testDataBlock)
            .set("Accept", "application/json")
            .expect(200)
